...
TMP_INSTALL_DIR="${TMPDIR:-/tmp}/brog-install"
...
RELEASE_FILE_PREFIX="brog_${OS_RELEASE}_${ARCH}"
...
BROG_BIN_TMP_PATH="${TMP_INSTALL_DIR}/brog"

ZIP_URL="https://github.com/PierreBeucher/brog/releases/latest/download/${ZIP_NAME}"
CHECKSUM_URL="https://github.com/PierreBeucher/brog/releases/latest/download/${CHECKSUM_NAME}"

echo "Downloading brog release..."
...
if [ "$(id -u)" -eq 0 ]; then
    ...
    mkdir -p "$INSTALL_DIR"
    mv "${BROG_BIN_TMP_PATH}" "${INSTALL_DIR}"
else
    echo "Copying to ${INSTALL_DIR}... (you may be prompted for sudo password)"
    sudo mkdir -p "$INSTALL_DIR"
    sudo mv "${BROG_BIN_TMP_PATH}" "${INSTALL_DIR}"
fi
...
case ":${PATH}:" in
    *:${INSTALL_DIR}:*)
        brog --version
        echo "brog has been successfully installed ✨"
        ;;
    *)
        echo -e "${YELLOW}Warning: ${INSTALL_DIR} is not in your PATH, brog commands may not work.${RESET}" >&2
        ;;
esac
